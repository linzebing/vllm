...
def test_mm_prefix_caching():
    ...
    assert block_hashes[0] == sha256((reloaded_kv_cache_utils.NONE_HASH,
                                      all_token_ids[:block_size], ("aaa", )))
    assert block_hashes[1] == sha256(
        (reloaded_kv_cache_utils.NONE_HASH,
         all_token_ids[block_size:block_size * 2], ("aaa", "bbb")))
    assert block_hashes[2] == sha256(
        (reloaded_kv_cache_utils.NONE_HASH,
         all_token_ids[block_size * 2:block_size * 3], ("bbb", )))
